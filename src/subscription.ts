...
export default class SubscriptionContract extends Deployed {
   ...
   * @param d - array of parameters and their values
   * @param priceUpdateData - Array of price update data(Pyth Oracle Data)
   * @param gasLimit - gas limit for tx
   */
  async makeChargeMantleWithGasLimit(u: string, d: Array<SubscriptionParameters>, priceUpdateData: string[], gasLimit: number): Promise<TxResponse> {
    const paramArray: Array<string> = []
    const paramValue: Array<number> = []
    for (let i = 0; i < d.length; i++) {
      paramArray.push(d[i].param)
